...
class NaiveCollisionDetectionEngine:
    ...
    def calculate_balls_balls_colls(self, balls: List[Ball], dt: float) -> Iterable[CollisionBallBall]:
    ...
    def calculate_balls_blocks_colls(self, balls: List[Ball], blocks: List[Block], dt: float) -> Iterable[CollisionBallBlock]:
        for ball, block in product(balls, blocks):
            yield from calculate_ball_block_colls(ball, block, dt)


    def calculate_balls_platforms_colls(self, balls: List[Ball], platforms: List[Platform], dt: float) -> Iterable[CollisionBallPlatform]:
        for ball, platform in product(balls, platforms):
            yield from calculate_ball_platform_colls(ball, platform, dt)


    def calculate_balls_walls_colls(self, balls: List[Ball], walls: List[Wall], dt: float) -> Iterable[CollisionBallWall]:
        for ball, wall in product(balls, walls):
            yield from calculate_ball_wall_colls(ball, wall, dt)


    def calculate_platforms_walls_colls(self, platforms: List[Platform], walls: List[Wall], dt: float) -> Iterable[CollisionPlatformWall]:
