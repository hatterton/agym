...
class Model:
    ...
    def move(self, arg):
        ...
        input_data = np.concatenate(
            (
                np.array(
                    [1.0, 20 * arg.ball.speed[0], 20 * arg.ball.speed[1]], float
                ).flatten(),
                input_data.flatten(),
            )
        )
        ...
        input_data = np.concatenate(
            (np.array(1.0, float).flatten(), input_data.flatten())
        )
    ...
    def dfs(self, rect, sol, x, y, value):
        ...
        v = [[-1, 0], [1, 0], [0, -1], [0, 1]]
        if (
            agym.param.left_side[x] < rect.right
            and agym.param.right_side[x] > rect.left
            and agym.param.top_side[y] < rect.bottom
            and agym.param.bottom_side[y] > rect.top
        ):
            sol[x, y] = value
            for vec in v:
    ...
    def crossover(self, other, arg):
        ...
        for i in range(2):
            ...
            sol.body[i] = (
                np.where(sel, a, a + (b - a) * np.random.rand()) * 0.99
            )
    ...
    def init_random(self):
        self.body[0] = (
            np.random.random([self.n_square + 1 + 2, agym.param.hidden_n]) - 0.5
        )
